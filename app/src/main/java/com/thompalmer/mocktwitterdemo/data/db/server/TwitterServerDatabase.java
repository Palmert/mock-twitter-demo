...
public class TwitterServerDatabase extends DatabaseInteractorImpl<TwitterServerDatabase.TwitterServerDbHelper> {
    public static final String TWITTER_SERVER_DB = "TwitterServerDb";

    @Inject
    public TwitterServerDatabase(Application application, SqlBrite sqlBrite) {
        super(new TwitterServerDbHelper(application), sqlBrite);
    }

    static class TwitterServerDbHelper extends SQLiteOpenHelper {

        private static final int DATABASE_VERSION = 1;
        private static final String DATABASE_NAME = "TwitterServer.db";
        private final Context context;

        TwitterServerDbHelper(Context context) {
